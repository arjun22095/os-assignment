...
void start_scheduling(process_queue *sq, int ncpu, int tslice, int *fd) {
	...
	while (rear_reached != 1) {
		...
		for (i = 0; i < ncpu; ++i) {
			...
			// dequeue the process from the scheduler queue
			cur_proc_node = dequeue(sq);
			procs_started[i] = cur_proc_node;
			cur_proc = cur_proc_node->proc;
			// increase the time it has executed by adding tslice to it
			cur_proc->exec_time += tslice;

			// this is for priority handling, it lets us know how many times
			// a particular process has been not sent at the back of the queue
